...
export class ItemUpgradeMenu {
    ...
    constructor (
        ...
        this.scene = scene
        this.item = item
        if (container) { this.outerContainer = container }
        this.type = type
        ...
        this.itemSprite = scene.add.image(10,menuHeight/2, item.sprite).setOrigin(0)
        this.statsText = scene.add.text(menuWidth-110, 10, this.getStatsText())
        // container for item data display
        this.itemMenu = scene.add.container(170, height-240).setActive(false).setVisible(false)
    ...
    private getStatsText = ():string => {
        let statsText = ''
        const { health, shield, bulletDamage, bulletRange, bulletSpeed, level, shootDelay, battery } = this.item
        ...
        if (this.type == 'equipment'){
            statsText = `
                ...
                \nInterval: ${shootDelay * level / ShipConstants.shootDelayDivisor}
                \nRange: ${bulletRange}
                \nBattery: ${battery}
            `
        } else {
            statsText = `
                ...
                \nShield: ${shield * (1+((level-1) * ShipConstants.shieldPerLevel / ShipConstants.shieldStatDivisor))}%
                \nDamage: ${bulletDamage * (1+((level-1) * ShipConstants.damagePerLevel))}
                \nSpeed: ${bulletSpeed}
                \nInterval: ${shootDelay}
                \nRange: ${bulletRange}
                \nBattery: ${battery}

...
export default class InventoryMenuScene extends Phaser.Scene {
    ...
    private inventory!: (InventoryItem | false)[];
    ...
    init(data: {
        profileData: PlayerWithInventory;
        type: "Equipment" | "Ships";
    }) {
    ...
    create() {
        ...
        this.pageLeft = this.add
            .text(menuWidth / 2 - 30, menuHeight - 12, "<")
            .setOrigin(0.5)
        ...
        this.pageRight = this.add
            .text(30 + menuWidth / 2, menuHeight - 12, ">")
            .setOrigin(0.5)
            .setInteractive({ useHandCursor: true })
            .on(
                "pointerdown",
        ...
        this.pageText = this.add
            .text(menuWidth / 2, menuHeight - 12, `1/${this.maxPage}`)
            .setOrigin(0.5);
        this.closeBtn = this.add
            .text(menuWidth - 15, 10, "X")
            .setOrigin(0.5)
            .setInteractive({ useHandCursor: true })
            .on(
                "pointerdown",
        ...
        //     (item, i) => item != false && this.menuContainer.add(item)
        // );
    }
    ...
    changePage = (direction: number) => {
        ...
        if (
            (this.page == 1 && direction < 0) ||
            (this.page == this.maxPage && direction > 0)
        ) {
            return;
        } else {
            this.page = page;
